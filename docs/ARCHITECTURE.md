...
What persists between turns differs by provider, because the two are stateful in different places. The Codex thread lives server-side and remembers everything ever sent to it, so `assistant-codex.cjs` sends the manifest only when it has changed: it keeps a hash of the last manifest sent on the current thread and includes the manifest in a turn only when the new hash differs. That hash is cleared everywhere the thread itself is cleared — construction, `reset()`, `dispose()`, and the fatal-error handler — so a new thread always gets a manifest, and hashing failures fail toward sending it rather than skipping it. DeepSeek's API is stateless and resends the full conversation on every request, so `assistant-deepseek.cjs` instead limits what it retains: `assistant-history.cjs` reduces each completed turn to its user question and final answer, dropping tool calls and tool results, then drops whole oldest exchanges until the history holds at most `MAX_HISTORY_EXCHANGES` (20) exchanges and `MAX_HISTORY_BYTES` (16384) bytes, never below one exchange. When that trim removes anything, the assembled request carries one `system` message right after the manifest noting that earlier turns were omitted.
